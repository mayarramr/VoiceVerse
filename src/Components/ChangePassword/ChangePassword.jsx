        ...
        <div className="position-fixed bgDark h-100 w-100">
            <div className="container d-flex flex-column justify-content-center align-items-center">
                <div className="w-50 d-flex justify-content-center">
                    <img src={change} className="w-25 my-4" alt="" />
                </div>
                <div className="row w-50 bgLightDark p-5 rounded-5">
                    <div className="textBlue text-center">
                       ...
                       {error ? <div className="alert alert-danger text-capitalize fs-5 py-2">{error}</div> : null}
                        {msg ? <div className="alert alert-success text-capitalize fs-5 py-2">{msg}</div> : null}
                    </div>
                    <form action="" onSubmit={changePasswordFormik.handleSubmit}>
                        <div className="position-relative">
                            ...
                            <div onClick={() => setvisible(!visible)} className="p-2 position-absolute top-50 translate-middle-y end-0">{visible ? <><FaEye /></> : <><FaEyeSlash className="fs-5 text-white" /></>}</div>
                        </div>
                        <div className="position-relative my-2">
                            <input type={visible ? "text" : "password"} placeholder="cPassword" name="cPassword" id="cPassword" className={`${Style.input} w-100 fs-5`} onChange={changePasswordFormik.handleChange} onBlur={changePasswordFormik.handleBlur} />
                            ...
                            <div onClick={() => setCVisible(!cVisible)} className="p-2 position-absolute top-50 translate-middle-y end-0">{cVisible ? <><FaEye /></> : <><FaEyeSlash className="fs-5 text-white" /></>}</div>
                        </div>
